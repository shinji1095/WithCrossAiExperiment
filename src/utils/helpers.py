...
def display_calculate_time(max_epoch,):
    def _display_calculate_time(func):
        def wrapper(*args, **kwargs):
            ...
            max_time = (max_epoch - args[0]) * elapsed_time
            hour = int(math.floor(max_time // 3600))
            minute = int(math.floor(max_time % 3600 // 60))
            second = int(math.floor(max_time % 3600 % 60))
            print(f"Rest of calculation time                                   : {hour:02d}:{minute:02d}:{second:02d}\n")
        return wrapper
    return _display_calculate_time